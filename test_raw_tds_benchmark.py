#!/usr/bin/env python
"""
Benchmark Python-side TDS serialization - 10 runs for statistical analysis.
"""
...
def run_single_test():
    """Run a single TDS serialization bulkcopy test and return throughput."""
    ...
    # Generate test data - Use 1KB strings to test string serialization performance
    ...
    # Import Core TDS module and bulk insert
    ...
    try:
        ...
        result = core_cursor.write_to_server_raw(
            ...
            kwargs={"batch_size": 0, "timeout": 60, "column_mappings": column_mappings}
        )
    ...
    time_per_row = (elapsed * 1_000_000) / rows_copied
    
    return throughput, time_per_row, elapsed


def main():
    print("Python TDS Serialization Benchmark")
    print("=" * 70)
    ...
    print(f"{'='*70}")
    print(f"\nThroughput (rows/s):")
    print(f"  Average:       {statistics.mean(throughputs):>10,.0f}")
    print(f"  Median:        {statistics.median(throughputs):>10,.0f}")
    print(f"  Min:           {min(throughputs):>10,.0f}")
    print(f"  Max:           {max(throughputs):>10,.0f}")
    if len(throughputs) > 1:
        print(f"  Std Dev:       {statistics.stdev(throughputs):>10,.0f}")
    
    print(f"\nTime per row (µs):")
    print(f"  Average:       {statistics.mean(times_per_row):>13.2f}")
    print(f"  Median:        {statistics.median(times_per_row):>13.2f}")
    print(f"  Min:           {min(times_per_row):>13.2f}")
    print(f"  Max:           {max(times_per_row):>13.2f}")
    if len(times_per_row) > 1:
        print(f"  Std Dev:       {statistics.stdev(times_per_row):>13.2f}")
    
    print(f"\nTotal time (s):")
    print(f"  Average:       {statistics.mean(total_times):>13.3f}")
    print(f"  Median:        {statistics.median(total_times):>13.3f}")
    print(f"  Min:           {min(total_times):>13.3f}")
    print(f"  Max:           {max(total_times):>13.3f}")
    if len(total_times) > 1:
        print(f"  Std Dev:       {statistics.stdev(total_times):>13.3f}")
